...
final class DeferredArtisanMigrationTest
{
    #[Test]
    public function deferredExecutionAndFailuresSurviveMigration(): void
    {
        $root = dirname(__DIR__, 4);
        $temporary = sys_get_temp_dir().'/laratesto-deferred-artisan-'.bin2hex(random_bytes(6));
        mkdir($temporary.'/corpus', 0777, true);
        $file = $temporary.'/corpus/DeferredArtisanTest.php';
        copy($root.'/tests/Fixture/issue10/DeferredArtisanTest.php', $file);
        $environment = ['LARATESTO_ISSUE10_APP' => $root.'/tests/Fixture/laravel', 'APP_ENV' => 'testing'];
        try {
            $sourceReport = simplexml_load_file($root.'/tests/Fixture/issue10/junit/DeferredArtisanTest.junit.xml');
            ...
            Assert::string($first)->contains('pendingArtisan');
            ...
            Assert::same(['total' => 5, 'passed' => 3, 'failed' => 2], $report['totals'], json_encode($report['failures']));
            $failures = array_column($report['failures'], 'test');
            sort($failures);
            Assert::same(['Tests\DeferredArtisanTest::testWrongExitCodeStillFailsOnRelease', 'Tests\DeferredArtisanTest::testWrongOutputStillFailsOnRelease'], $failures);
        } finally {
