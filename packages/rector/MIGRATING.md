...
| `DATABASE_UNSUPPORTED_CONFIGURATION` | custom `beforeRefreshingDatabase()` hooks, dynamic `$connectionsToTruncate`, an option property such as `$seed` or `$connectionsToTransact` declared on a resolved project ancestor (unless the class itself redeclares the same option — the child's literal is then the only live value) or supplied by a used project trait (directly or through an ancestor; still live there through `property_exists()`, but out of reach of the class-level attribute), the same database trait re-declared on a descendant of a class that already uses it with a different configuration, a used trait that cannot be resolved | move the hook body into the test or `setUpLaravel()`, express options as literal attribute arguments on the class that carries the trait, and drop the duplicated trait use so the hierarchy inherits the single attribute |
